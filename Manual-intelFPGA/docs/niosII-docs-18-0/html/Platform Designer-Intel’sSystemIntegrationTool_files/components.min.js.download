...
this.video=this.videoWrapper.firstChild;
...
this.initSettings(this.videoID)
},modalClose:function(c){if(!this.playFromModal){this.srcElem.setAttribute("src","");
this.srcElem.setAttribute("type","")
}this.video.load()
}},b={videoWrapper:null,init:function(){this.videoWrapper=document.querySelectorAll(".video-js");
this.setup()
},setup:function(){var d=[].slice.call(this.videoWrapper),c;
d.forEach(function(e){if(e.firstChild.dataset.playFromLink==="true"||e.firstChild.dataset.playFromModal==="true"){c=Object.create(a,{videoWrapper:{value:e},initSettings:{value:this.initSettings},videoSettings:{value:this.videoSettings},playFromModal:{value:e.firstChild.dataset.playFromModal}});
c.init()
}else{this.initSettings(e.id)
}}.bind(this))
},initSettings:function(c){videojs(c).ready(this.videoSettings(c))
},videoSettings:function(e){var d=$(this),c=$("#"+e);
c.closest("video-js").on("resize",function(f){AlteraVideo.resize(c)
});
AlteraVideo.resize(c);
d.on("fullscreenchange",function(f){AlteraVideo.resize(c)
});
d.on("volumechange",function(f){AlteraVideo.handleMute(c)
})
}};
b.init()
});
...
}this.initDataTables=function(o,a,u,c,g){var t=window.location.host;
if(t.indexOf("co.jp")!==-1){Granite.I18n.setLocale("ja")
}else{if(t.indexOf("com.cn")!==-1){Granite.I18n.setLocale("zh")
}else{Granite.I18n.setLocale("en")
}}var j=Granite.I18n.get("First"),k=Granite.I18n.get("Last"),q=Granite.I18n.get("Next"),p=Granite.I18n.get("Prev"),l=Granite.I18n.get("noData"),e=Granite.I18n.get("Showing"),d=Granite.I18n.get("Filtered From"),h=Granite.I18n.get("Entries"),v=Granite.I18n.get("total"),f=Granite.I18n.get("noEntries"),b='<"top"fli>rt<"table-controls-bottom"p><"clear">',n=[[0,"asc"]],s=[];
$.fn.DataTable.ext.pager.numbers_length=7;
if(a){b="B"+b
}if(c){n=[]
}if(g){s=g
}var m=$(o).DataTable({buttons:["excel"],order:n,pageLength:10,language:{url:"",aria:{sSortAscending:": activate to sort column ascending",sSortDescending:": activate to sort column descending"},search:"",paginate:{first:"<<",last:">>",next:">",previous:"<"},emptyTable:l,info:e+" _START_ - _END_ / _TOTAL_ "+h,infoEmpty:e+" 0 - 0 / 0 "+h,infoFiltered:"("+d+" _MAX_ "+v+" "+h+")",zeroRecords:f},displayLength:20,columnDefs:s,dom:b,buttons:[{text:"Download",extend:"csvHtml5",className:"export-csv",title:u,extension:".csv"}],paginationType:"full_numbers"}),r;
setTimeout(function(){r=$(".input-clear");
r.on("click",function(w){m.search("").columns().search("").draw()
})
},0);
$(o).wrap("dataTables_scrollHead");
return m
}
};
...
}$.post("/bin/cloudservice",p,function(w){if(s!==undefined){s(w)
...
$.post("/bin/cloudservice",p,function(v){if(s!==undefined){s(v)
