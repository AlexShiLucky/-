...
videojs.plugin("ga",function(e){var k,l,g,o,m,i,v,d,p,r,h,t,n,u,w,f,j,c,b,q,s;
if(e==null){e={}
}k={};
if(this.options()["data-setup"]){r=JSON.parse(this.options()["data-setup"]);
if(r.ga){k=r.ga
}}l=["loaded","percentsPlayed","start","end","seek","play","pause","resize","volumeChange","error","fullscreen"];
v=e.eventsToTrack||k.eventsToTrack||l;
n=e.percentsPlayedInterval||k.percentsPlayedInterval||10;
m=e.eventCategory||k.eventCategory||"Video";
i=e.eventLabel||k.eventLabel;
e.debug=e.debug||false;
t=[];
j=f=0;
c=false;
p=function(){if(!i){i=this.currentSrc().replace(/\/jcr:content.*/i,"")
}if(a.call(v,"loadedmetadata")>=0){b("loadedmetadata",true)
}};
q=function(){var z,B,y,x,A;
z=Math.round(this.currentTime());
B=Math.round(this.duration());
x=Math.round(z/B*100);
for(y=A=0;
A<=99;
y=A+=n){if(x>=y&&a.call(t,y)<0){if(a.call(v,"start")>=0&&y===0&&x>0){b("start",true)
}else{if(a.call(v,"percentsPlayed")>=0&&x!==0){b("percent played",true,y)
}}if(x>0){t.push(y)
}}}if(a.call(v,"seek")>=0){j=f;
f=z;
if(Math.abs(j-f)>1){c=true;
b("seek start",false,j);
b("seek end",false,f)
}}};
g=function(){b("end",true)
};
u=function(){var x;
x=Math.round(this.currentTime());
b("play",true,x);
c=false
};
h=function(){var x,y;
x=Math.round(this.currentTime());
y=Math.round(this.duration());
if(x!==y&&!c){b("pause",false,x)
}};
s=function(){var x;
x=this.muted()===true?0:this.volume();
b("volume change",false,x)
};
w=function(){b("resize - "+this.width()+"*"+this.height(),true)
};
o=function(){var x;
x=Math.round(this.currentTime());
b("error",true,x)
...
d=function(){var x;
x=Math.round(this.currentTime());
if((typeof this.isFullscreen==="function"?this.isFullscreen():void 0)||(typeof this.isFullScreen==="function"?this.isFullScreen():void 0)){b("enter fullscreen",false,x)
}else{b("exit fullscreen",false,x)
}};
b=function(y,z,x){if(window.ga){ga("send","event",{eventCategory:m,eventAction:y,eventLabel:i,eventValue:x,nonInteraction:z})
}else{if(window._gaq){_gaq.push(["_trackEvent",m,y,i,x,z])
}else{if(e.debug){console.log("Google Analytics not detected")
}}}};
this.ready(function(){this.on("loadedmetadata",p);
this.on("timeupdate",q);
if(a.call(v,"end")>=0){this.on("ended",g)
}if(a.call(v,"play")>=0){this.on("play",u)
}if(a.call(v,"pause")>=0){this.on("pause",h)
}if(a.call(v,"volumeChange")>=0){this.on("volumechange",s)
}if(a.call(v,"resize")>=0){this.on("resize",w)
}if(a.call(v,"error")>=0){this.on("error",o)
}if(a.call(v,"fullscreen")>=0){return this.on("fullscreenchange",d)
}});
return{sendbeacon:b}
})
}).call(this);
AlteraVideo=(typeof AlteraVideo!="undefined")?AlteraVideo:{};
AlteraVideo.resize=function(h){var b=($(h).is("div"))?$(h):$(h).parent();
var c=b.width();
var f=23;
var e=23;
var a=25;
...
AlteraVideo.handleMute=function(d){var b=($(d).is("div"))?$(d):$(d).parent();
var c=b.find(".vjs-control-bar");
var a=c.find(".vjs-mute-control");
if(!a.hasClass("vjs-vol-0")){c.removeClass("vjs-vol-0")
}else{c.addClass("vjs-vol-0")
}};
$(function(){$("video").each(function(){var a=$(this).attr("id");
videojs(a).ready(function(){var b=this;
b.ga()
})
})
});