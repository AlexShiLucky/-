...
(function(){var a=!!window.MSInputMethodContext&&!!document.documentMode,b=navigator.appVersion.indexOf("MSIE 10")!==-1,d=navigator.appVersion.indexOf("MSIE 9")!==-1,c=document.createElement("script");
if(a||b||d){c.onload=function(){console.log("loaded")
};
c.type="text/javascript";
c.src="/etc/clientlibs/altera-www/global/js/polyfills/polyfills.min.js";
...
svg4everybody();
$(document).ready(function(){var b=document.getElementById("navbar");
if(!b){return
}var j=document.getElementById("search")||null,e=document.querySelector(".button-overlay")||null,c=document.querySelector("#feedback_btn")||null,a=document.querySelector("#feedback")||null,g=j.querySelector(".morph-search")||null,i="";
(function(){var k={mega_menu:document.getElementById("mega_menu"),mega_menu_nav:document.getElementById("mega_menu_nav"),mega_menu_items_wrapper:document.getElementById("mega_menu_items_wrapper"),init:function(){document.addEventListener("click",this.events.bind(this),false)
},events:function(m){var l=m.target;
if(l.classList.contains("menu")){document.documentElement.classList.add("modal-open");
this.mega_menu.classList.add("overlay");
setTimeout(function(){this.mega_menu.classList.add("expand")
},0)
}if(l.classList.contains("nav-link")){m.preventDefault();
var n=l.hash.substring(1);
this.selectMenuItem(n)
}},selectMenuItem:function(n){var m=this.mega_menu_nav.querySelector(".selected")||null,l=this.mega_menu_nav.querySelector("."+n);
if(m){m.classList.remove("selected")
}l.classList.add("selected");
this.showMenuItems(n)
},showMenuItems:function(n){var l=this.mega_menu_items_wrapper.querySelector(".selected"),m=document.getElementById(n);
...
function f(l){var k=l.target;
if(k.className==="search-box"||k.className==="search-submit"||k.classList.contains("search-icon-btn")){k.preventDefault;
i=$(g);
setTimeout(function(){i.css({position:"fixed"})
},120);
document.documentElement.classList.add("modal-open");
g.classList.add("expand")
}if(k.classList.contains("overlay-close")&&document.documentElement.classList.contains("modal-open")){if(g.classList.contains("expand")){h()
}d(k);
document.documentElement.classList.remove("modal-open")
}if(k.classList.contains("overlay-close")&&k.parentElement.classList.contains("login-overlay")){d(k)
}if(k.classList.contains("feedback-btn")){document.documentElement.classList.add("modal-open");
setTimeout(function(){a.classList.add("expand")
},0)
}if(k.classList.contains("button-overlay-init")){document.documentElement.classList.add("modal-open");
e.classList.add("overlay");
setTimeout(function(){e.classList.add("expand")
},0)
}}function h(){var l=i.find(".ss-gac-m"),k=i.find(".search-box");
if(l.is(":visible")){l.css("visibility","hidden")
}if(k.val()!==""){k.val("")
}setTimeout(function(){i.css({position:"absolute"})
},300)
}function d(l){var k=l.parentNode;
while(!k.classList.contains("overlay")){k=k.parentNode
}k.classList.remove("expand");
if(k.classList.contains("button-overlay")){setTimeout(function(){k.classList.remove("overlay")
},300)
}}});
(function(){$(document).ready(function(){if(!document.querySelector(".sidebar-widgets")){return
}var c=document.querySelector(".sidebar-widgets")||null,b=c.getAttribute("data-fixed-sidebar")||true;
if(c&&b&&b!=="false"){$(window).on("resize",fpga.utils.debounce(d,100));
d()
}function a(){this.bottom=$(".main-footer").outerHeight(true);
return(this.bottom)
}function d(){var e=$(c);
c.style.height=window.innerHeight-90+"px";
if(window.matchMedia("(min-width: 768px)").matches){e.affix({offset:{top:160,bottom:a}})
}else{return
}}})
}());
...
(function(){function b(e,d){var c={root:document.querySelector(e),sidebarFixed:null,init:function(){this.windowWidth=window.innerWidth;
this.content=document.querySelector(".sidebar-content-container");
this.handle=this.root.querySelector(".sidebar-collapse-handle")||d.handle;
this.handle.addEventListener("click",this.handleEvents.bind(this),false);
this.sidebarFixed=JSON.parse(this.root.querySelector(".sidebar-widgets").getAttribute("data-fixed-sidebar"));
if(this.sidebarFixed){$(window).on("resize",fpga.utils.debounce(this.checkSize.bind(this),100));
this.checkSize()
...
c.init()
}function a(){var e=document.querySelector(".sidebar-widgets"),c=e.offsetHeight,d=document.querySelector(".sidebar-content-container");
d.style.minHeight=c+"px"
}$(document).ready(function(){if(document.querySelector(".sidebar-collapse")){setTimeout(a,0);
b(".sidebar-collapse")
}})
}());
$(window).load(function(){(function a(){var b={root:document.getElementById("feedback"),feedbackBtn:document.getElementById("feedback_btn"),submitBtn:document.getElementById("feedback_submit"),feedbackForm:null,currentUrl:document.getElementById("feedback_current_url"),previousUrl:document.getElementById("feedback_previous_url"),init:function(){if(!this.root){return
}this.feedbackBtn.addEventListener("click",this.handleEvents.bind(this),false);
this.root.addEventListener("click",this.submitFeedback.bind(this),false);
this.previousUrl.value=document.referrer;
this.currentUrl.value=window.location.href
},handleEvents:function(c){this.feedbackForm=document.getElementById("feedback_form");
this.thankYou=document.getElementById("feedback_success");
this.errorMsg=document.getElementById("feedback_error");
this.validationMsg=document.getElementById("feedback_validation")
},submitFeedback:function(h){var d=false,g=h.target,c="/bin/feedback",f=$(this.feedbackForm).serializeArray();
h.preventDefault;
$("input.required").each(function(){if($(this).val()===""){d=true
}});
$("textarea.required").each(function(){if($(this).val()===""){d=true
}});
if(d){return
}if(g===this.submitBtn){$.ajax({url:c,type:"POST",data:f,success:this.success.bind(this),error:this.error.bind(this)})
}return false
},success:function(d,e,c){if(d==="success"){this.feedbackForm.classList.add("hide");
this.validationMsg.classList.add("show");
this.validationMsg.innerHTML="";
this.thankYou.classList.add("show")
}else{this.validationMsg.classList.add("show");
this.validationMsg.innerHTML="";
this.validationMsg.innerHTML=d
}},showError:function(c){this.feedbackForm.classList.add("hide");
...
fpga.mainSearch={};
"use strict";
$(document).ready(function(){var g=window.location.pathname,b=document.getElementById("gsaSite"),h=document.getElementById("gsaMainsite"),d=document.getElementById("gsaAccess"),f=document.getElementById("gsaSiteValue"),a=document.getElementById("gsaMainSiteValue");
gsaSiteSepValue=document.getElementById("gsaSiteSepValue"),gsaMainSiteSepValue=document.getElementById("gsaMainSiteSepValue");
gsaAccessSepValue=document.getElementById("gsaAccessSepValue");
if(g.indexOf("documentation")===1){b.value=f.innerText;
h.value=a.innerText
}else{if(g.indexOf("sep")===1){b.value=gsaSiteSepValue.innerText;
h.value=gsaMainSiteSepValue.innerText;
d.value=gsaAccessSepValue.innerText
}else{var c=fpga.utils.getCookie("login-token-tracking");
var e=fpga.utils.getCookie("cugp");
if(c!==null&&c!==""&&e!==null&&e!==""){if(e!==null&&e!==""){if(e.indexOf("pub-sep-viewers")>=1){if(g.indexOf("bin/search")<1){d.value=gsaAccessSepValue.innerText
}}}}}}});
(function(){var a={state:{loggedIn:false},init:function(b){if(this.log_in_form_header===0){return
}this.login_btn=b||null,this.logout_btn=document.getElementById("after_login_logout")||null,this.locale=document.getElementById("intel_fpga").dataset.region;
this.login_btn.addEventListener("click",this.setButton.bind(this),false);
this.cookieusername=fpga.utils.getCookie("userName");
this.setLoginState()
},getServer:function(){var c=location.host.toLowerCase(),b=["localhost:4502","dev","qa"].filter(function(d){return c.indexOf(d)>-1?d:""
});
return b.join("")
},setLocale:function(){var b=this.locale,c=["en","jp","cn"].filter(function(d){return b.indexOf(d)>-1?d:""
});
return c.join("")
},setButton:function(h){var g=h.target,b=this.setLocale(),f=location.host.toLowerCase(),c=this.getServer(),d="";
if(!this.state.loggedIn){d="https://sso.altera.com/idp/startSSO.ping?PartnerSpId=myaltera"+c+"_"+b;
window.location=d
}if(this.state.loggedIn){this.showOptions();
this.setLogoutLink();
return
}},showOptions:function(c){var b=document.getElementById("login");
if(b.classList.contains("expand")){b.classList.remove("expand");
return
}b.classList.add("expand")
},setLogoutLink:function(){this.logout_btn.href=location.origin+"/system/sling/logout?resource=/content/altera-www/global/"+this.locale+"/index/myaltera"
},setLoginState:function(){var b=fpga.utils.getCookie("login-token-tracking");
if(b!==null&&b!==""&&this.cookieusername!==null&&this.cookieusername!==""){this.state.loggedIn=true
}else{this.state.loggedIn=false
}this.setIcon()
},setIcon:function(){var c=document.getElementById("login_default"),b=document.getElementById("login_welcome"),d;
if(this.state.loggedIn){d=this.cookieusername.replace(/\"/g," ");
b.classList.remove("hide");
c.classList.add("hide");
document.getElementById("logged_in_label").innerHTML=d;
document.getElementById("logged_in_label_overlay").innerHTML=d;
return
}b.classList.add("hide");
c.classList.remove("hide")
}};
fpga.pageLoad(function(){if(this.isPublish()&&document.getElementById("login_btn")){a.init(document.getElementById("login_btn"))
}})
}());
(function(){var a={root:document.getElementsByTagName("body")[0],clearCookie:fpga.utils.getCookie("clearCookie"),loggedOut:false,path:null,clearPath:"/bin/myAlteraCookieAuth?action=clear",href:null,domain1:null,domain2:null,url:window.location.href,setDomain:function(){if(this.url.indexOf("altera.com.cn")!==-1){this.domain1=this.href+".altera.com";
this.domain2=this.href+".altera.co.jp";
return
}if(this.url.indexOf("altera.co.jp")!==-1){this.domain1=this.href+".altera.com";
return
}},setCookieToken:function(){if(this.clearCookie!==null&&this.clearCookie!==""){this.loggedOut=true
}},buildImg:function(c){var b=document.createElement("img");
b.width="1";
b.height="1";
b.src=c+this.path;
b.className="hidden";
return b
},appendImg:function(b){this.root.appendChild(b)
},isLoggedOut:function(){var b=false;
if(this.loggedOut){b=true;
this.path=this.clearPath;
fpga.utils.createCookie("clearCookie","",".altera.com","0");
fpga.utils.createCookie("clearCookie","",".altera.com.cn","0");
fpga.utils.createCookie("clearCookie","",".altera.co.jp","0")
}if(b){this.appendImg(this.buildImg(this.domain1));
this.appendImg(this.buildImg(this.domain2))
}},init:function(){this.href=this.url.split(".")[0];
this.domain1=this.href+".altera.co.jp";
this.domain2=this.href+".altera.com.cn";
this.setCookieToken();
this.setDomain();
this.isLoggedOut()
}};
fpga.pageLoad(function(){if(this.isPublish()){a.init()
}})
}());
(function(){var a={init:function(){this.btn=document.getElementById("sep_home_btn");
if(!this.btn){return
}this.checkAccess()
},checkAccess:function(){if(this.hasAccess()){this.showbtn();
return
}this.hideBtn()
},hasAccess:function(){var c=fpga.utils.getCookie("cugp")||"",b=false;
if(c&&c.indexOf("pub-sep-viewers")>-1){b=true
}return b
},showbtn:function(){this.btn.classList.remove("hide")
},hideBtn:function(){this.btn.classList.add("hide")
}};
fpga.pageLoad(function(){if(this.isPublish()){a.init()
}})
}());
...
f({url:"/bin/dsnOptionList",data:"getDeviceFamily",el:$("#devices")});
$("#doctype").multiselect({click:function(i,j){var k=j.value;
if(j.checked){e(k,"doctype")
}else{b(k,"doctype")
}},header:false,noneSelectedText:"Select options",position:({my:"center top",at:"center bottom",collision:"flipfit",within:window})});
f({url:"/bin/optionListDoc",data:"doctype",el:$("#doctype")})
});
...
var h="http://wwwdev.altera.com/bin/rssdoc?devices="+r+"&doctype="+j;
...
var h="http://wwwdev.altera.com/bin/rssdoc?devices="+u+"&doctype="+k;
document.getElementById("copyTarget").value=h
