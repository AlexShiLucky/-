...
fpga.utils.createCookie=function(c,f,e,d){var a="",e="; domain="+e,b=null;
if(d){b=new Date();
b.setTime(b.getTime()+(d*1000));
a="; expires="+b.toGMTString()
}document.cookie=c+"="+f+a+e+";path=/"
};
fpga.utils.getCookie=function(b){var e=b+"=";
...
if(c.martch(b)){return c.replace(b,"$1"+a+"="+d+"$2")
}else{return c+e+a+"="+d
}};
svg4everybody();
$(document).ready(function(){var i=document.getElementById("search")||null,d=document.querySelector(".button-overlay")||null,b=document.querySelector("#feedback_btn")||null,a=document.querySelector("#feedback")||null,f=i.querySelector(".morph-search")||null,h="";
(function(){var j={mega_menu:document.getElementById("mega_menu"),mega_menu_nav:document.getElementById("mega_menu_nav"),mega_menu_items_wrapper:document.getElementById("mega_menu_items_wrapper"),init:function(){document.addEventListener("click",this.events.bind(this),false)
...
}},k={loginBtn:document.getElementById("login_overlay"),login:document.getElementById("login"),init:function(){this.loginBtn.addEventListener("click",this.handleEvents.bind(this),false)
},handleEvents:function(l){document.documentElement.classList.add("modal-open");
login.classList.add("overlay");
setTimeout(function(){login.classList.add("expand")
},0)
}};
k.init();
j.init()
...
if(j.className==="search-box"||j.className==="search-submit"||j.classList.contains("search-icon-btn")){j.preventDefault;
h=$(f);
setTimeout(function(){h.css({position:"fixed"})
},120);
document.documentElement.classList.add("modal-open");
f.classList.add("expand")
}if(j.classList.contains("overlay-close")){if(f.classList.contains("expand")){g()
}c(j);
document.documentElement.classList.remove("modal-open")
}if(j.classList.contains("feedback-btn")){document.documentElement.classList.add("modal-open");
setTimeout(function(){a.classList.add("expand")
},0)
}if(j.classList.contains("button-overlay-init")){document.documentElement.classList.add("modal-open");
d.classList.add("overlay");
setTimeout(function(){d.classList.add("expand")
},0)
}}function g(){var k=h.find(".ss-gac-m"),j=h.find(".search-box");
if(k.is(":visible")){k.css("visibility","hidden")
}if(j.val()!==""){j.val("")
}setTimeout(function(){h.css({position:"absolute"})
},300)
}function c(k){var j=k.parentNode;
while(!j.classList.contains("overlay")){j=j.parentNode
}j.classList.remove("expand");
if(j.classList.contains("button-overlay")){setTimeout(function(){j.classList.remove("overlay")
},300)
}}});
...
if(c&&b&&b!=="false"){$(window).on("resize",fpga.utils.debounce(a,100));
a()
}});
function a(){if(window.matchMedia("(min-width: 768px)").matches){$(".sidebar-widgets").affix({offset:{top:160,bottom:function(){return(this.bottom=$(".main-footer").outerHeight(true)+100)
}}})
}else{return
}}}());
...
(function(){function b(e,d){var c={root:document.querySelector(e),init:function(){this.windowWidth=window.innerWidth;
...
$(window).on("resize",fpga.utils.debounce(this.checkSize.bind(this),100));
this.checkSize()
...
$(window).load(function(){(function a(){var b={root:document.getElementById("feedback"),feedbackBtn:document.getElementById("feedback_btn"),submitBtn:document.getElementById("feedback_submit"),feedbackForm:null,init:function(){if(!this.root){return
...
},submitFeedback:function(h){var d=false;
$("input.required").each(function(){if($(this).val()==""){d=true
}});
$("textarea.required").each(function(){if($(this).val()==""){d=true
}});
if(d){return
}var g=h.target,c="/bin/feedback",f=$(this.feedbackForm).serializeArray();
h.preventDefault;
...
fpga.mainSearch={};
"use strict";
(function(){var b=window.location.pathname,a=document.getElementById("gsaSite"),c=document.getElementById("gsaMainsite");
if(b.indexOf("documentation")===1){a.value="www_documentation";
c.value="www_documentation"
}});
$(document).ready(function(){var a={log_in_form_header:document.getElementById("log_in_form_header")||null,state:{loggedIn:false},init:function(){if(this.log_in_form_header===0){return
}this.log_in_form_header.addEventListener("submit",this.submit.bind(this));
this.cookieusername=fpga.utils.getCookie("userName");
this.setLoginState()
},submit:function(c){c.preventDefault();
var f=$("#j_username_header").val(),b=$("#j_password_header").val(),d=document.getElementById("login_error");
if(f!==""&&f.length>0&&b!==""&&b.length>0){this.log_in_form_header.submit()
}else{alert(d.innerText)
}},stayOnPage:function(){fpga.utils.createCookie("targetResource",location.href,location.host,30)
},setLoginState:function(){var b=fpga.utils.getCookie("login-token");
if(b!==null&&b!==""&&this.cookieusername!==null&&this.cookieusername!==""){this.state.loggedIn=true
}else{this.state.loggedIn=false
}this.setIcon();
this.setWindow()
},setIcon:function(){var c=document.getElementById("login_default"),b=document.getElementById("login_welcome");
if(this.state.loggedIn){b.classList.remove("hide");
c.classList.add("hide");
document.getElementById("logged_in_label").innerHTML=this.cookieusername.replace(/"/g," ");
return
}b.classList.add("hide");
c.classList.remove("hide")
},setWindow:function(){var b=document.getElementById("login_section"),c=document.getElementById("logout_section");
if(this.state.loggedIn){b.classList.add("hide");
c.classList.remove("hide");
return
